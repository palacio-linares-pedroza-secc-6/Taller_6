...
public class InterfazLibreria extends JFrame {
	...
	public void cargarArchivos(File archivo_categorias, File archivo_libros) {
		try {
			...
			boolean ans = libreria.darNuevasCategorias().isEmpty();

			if (ans == false) {
				ArrayList<String> nuevas = libreria.darNuevasCategorias();
				String nuevasS = "";

				for (int i = 0; i < nuevas.size(); i++) {
					if (i == 0) {
						nuevasS += " " + nuevas.get(i);
					} else {
						nuevasS += ", " + nuevas.get(i);
					}
				}

				JOptionPane.showMessageDialog(panelCategorias, "Las nuevas categorias son:" + nuevasS + ".");
			}
		} catch (Exception e) {
	...
	public void RenombrarCategoria() {
		String titulo = JOptionPane.showInputDialog(this, "Escriba el nombre de la categoria a renombrar", "categoria");
		String nombre = JOptionPane.showInputDialog(this, "Escriba el nuevo nombre", "nombre");
		if (titulo != null && nombre != null) {
			Categoria categoria = libreria.buscarExistencia(titulo);
			if (categoria == null) {
				JOptionPane.showMessageDialog(this, "No se encontr� dicha categoria", "No hay categoria",
						JOptionPane.INFORMATION_MESSAGE);
			} else {
				// funcion que cambie el nombre pero que si hay dos iguales no se bugee
				if (libreria.cambioNombre(categoria, nombre) == false) {
					JOptionPane.showMessageDialog(this,
							"No se pudo cambiar el nombre, el nombre que se propuso ya existe", "No se pudo",
							JOptionPane.INFORMATION_MESSAGE);
				} else {
					JOptionPane.showMessageDialog(this, "Se cambi� el nombre con �xito", "Cambio ocurrido!",
							JOptionPane.INFORMATION_MESSAGE);
				}
			}
		}
	...
	public void borrarLibros() throws Exception {
		String autores = JOptionPane.showInputDialog(this,
				"Escriba el nombre del autor o autores que quiere borrar sus libros \n(separe los autores con ,)",
				"autor");
		if (autores != null) {
			try {
				int removidos = libreria.borrarLibros(autores);
				Categoria currentCategoria = this.panelCategorias.darCategoria();
				this.cambiarCategoria(currentCategoria);
				JOptionPane.showMessageDialog(this, "Se removieron un total de: " + removidos + " libros", "Exito",
						JOptionPane.INFORMATION_MESSAGE);

			} catch (Exception e) {
				JOptionPane.showMessageDialog(this, e.getMessage(), "Autor sin libros", JOptionPane.ERROR_MESSAGE);
			}
		}
	}
